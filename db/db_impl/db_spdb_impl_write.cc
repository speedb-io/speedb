...
static constexpr size_t kFlushCheckTimeoutInSecs = 5;
...
void SpdbWriteImpl::SpdbFlushWriteThread() {
  for (;;) {
    ...
    flush_thread_cv_.wait_for(lck,
                              std::chrono::seconds(kFlushCheckTimeoutInSecs));
    if (flush_thread_terminate_) {
      break;
    }
    if (db_->IsSpdbWrite()) {
      if (flush_needed_.load() || db_->IsFlushNeeded()) {
        WriteLock wl(&flush_rwlock_);
        db_->RegisterFlush();
        flush_needed_.store(false);
      }
    }
  }
}
...
SpdbWriteImpl::SpdbWriteImpl(DBImpl* db)
    : db_(db), flush_thread_(&SpdbWriteImpl::SpdbFlushWriteThread, this) {
...
void SpdbWriteImpl::NotifyFlushNeeded() {
  bool tmp = false;
  if (flush_needed_.compare_exchange_strong(tmp, true,
                                            std::memory_order_relaxed)) {
    flush_thread_cv_.notify_one();
  }
}

void DBImpl::NotifyFlushNeeded() {
  if (immutable_db_options_.use_spdb_writes) spdb_write_.NotifyFlushNeeded();
}

Status DBImpl::RegisterFlush() {
  Status status;
...
Status DBImpl::SpdbWrite(const WriteOptions& write_options,
  ...
  spdb_write_.GetFlushRWLock().ReadLock();
  ...
  spdb_write_.GetFlushRWLock().ReadUnlock();
