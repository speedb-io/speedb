...
void SpdbWriteImpl::WritesBatchList::WriteBatchComplete(bool leader_batch,
  ...
  if (leader_batch) {
    // make sure all batches wrote to memtable (if needed) to be able progress the version
...
void SpdbWriteImpl::WritesBatchList::WaitForPendingWrites() {
  // make sure all batches wrote to memtable (ifneeded) to be able progress the version  
  WriteLock wl(&write_ref_rwlock_);  
}
...
void* SpdbWriteImpl::Add(WriteBatch* batch, bool disable_wal, bool* leader_batch) {
...
void SpdbWriteImpl::CompleteMerge() {
  add_buffer_mutex_.Unlock(); 
}
  ...
  if (!batch_group.wal_writes_.empty()) {
    ...
    if (batch_group.wal_writes_.size() == 1 && batch_group.wal_writes_.front()
      ...
      for (const WriteBatch* batch : batch_group.wal_writes_) {
        if (wal_writes != 0 && (progress_batch_seq != WriteBatchInternal::Sequence(batch))) {
        ...
        if (wal_writes == 0) {
          ...
          WriteBatchInternal::SetSequence(merged_batch, WriteBatchInternal::Sequence(batch));
        } 
    ...
    if (!io_s.ok()) {
      // TBD what todo with error
      ROCKS_LOG_ERROR(db_->immutable_db_options().info_log, "Error write to wal!!! %s", io_s.ToString().c_str());
    }
  }
...
Status DBImpl::SpdbWrite(const WriteOptions& write_options,
  ...
  if (batch->HasMerge()) {
    // need to wait all prev batches completed to write to memetable and avoid
    // new batches to write to memetable before this one
    list = spdb_write_->AddMerge(batch, 
                                write_options.disableWAL, 
                                &leader_batch);
  } else {
    list = spdb_write_->Add(batch, 
                          write_options.disableWAL, 
                          &leader_batch);
  }
