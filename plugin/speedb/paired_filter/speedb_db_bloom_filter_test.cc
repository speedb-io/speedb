...
std::shared_ptr<const FilterPolicy> Create(double bits_per_key,
  ...
  if (name == SpdbPairedBloomFilterPolicy::kClassName()) {
    return std::make_shared<SpdbPairedBloomFilterPolicy>(bits_per_key);
  } else {
    return nullptr;
  }
}
const std::string kSpdbPairedBloom = SpdbPairedBloomFilterPolicy::kClassName();
...
class SliceTransformLimitedDomainGeneric : public SliceTransform {
  ...
  Slice Transform(const Slice& src) const override {
    return Slice(src.data(), kPrefixLen);
  }

  bool InDomain(const Slice& src) const override {
    // prefix will be x????
    return src.size() >= kPrefixLen;
  }

  bool InRange(const Slice& dst) const override {
    // prefix will be x????
    return dst.size() == kPrefixLen;
  }
};
...
TEST_P(SpdbDBBloomFilterTestDefFormatVersion, KeyMayExist) {
  do {
    ReadOptions ropts;
    std::string value;
...
TEST_P(SpdbDBBloomFilterTestWithParam,
  ...
  ASSERT_EQ(
      1, (*(get_perf_context()->level_to_perf_context))[0].bloom_filter_useful);

  ASSERT_EQ("NOT_FOUND", Get("foobarbar"));
  ASSERT_EQ(TestGetTickerCount(options, BLOOM_FILTER_USEFUL), 2);
  ASSERT_EQ(
      2, (*(get_perf_context()->level_to_perf_context))[0].bloom_filter_useful);
...
TEST_P(SpdbDBBloomFilterTestWithParam, BloomFilter) {
  do {
    ...
    const auto kBpk = 20U;
    const auto bytes_per_key = kBpk / 8;
    table_options.filter_policy = Create(kBpk, kSpdbPairedBloom);
    ASSERT_FALSE(table_options.filter_policy == nullptr);
    table_options.partition_filters = partition_filters_;
    if (partition_filters_) {
      table_options.index_type =
          BlockBasedTableOptions::IndexType::kTwoLevelIndexSearch;
    }
    ASSERT_GE(table_options.format_version, 5U);
    ...
    // TODO: Our Filter has a min size of 8192 bytes (64 X 128) => The upper
    // limit depends on the number of filters
    // => Adapt the caclulation
    // // // EXPECT_LE(filter_size,
    // // //           (partition_filters_ ? 12 : 11) * nkeys / /*bits / byte*/
    // 8); Always Bloom
    EXPECT_GE(filter_size, static_cast<uint64_t>(bytes_per_key * nkeys));
...
TEST_P(SpdbDBBloomFilterTestWithParam, SkipFilterOnEssentiallyZeroBpk) {
  ...
  // constructed or read.
  table_options.filter_policy = Create(0.4, kSpdbPairedBloom);
  options.table_factory.reset(NewBlockBasedTableFactory(table_options));
  DestroyAndReopen(options);
  PutAndGetFn();
...
// // //   // Create one file for each kind of filter. Each file covers a
// distinct key
// // //   // range.
...
// // //   // Test filter is used between each pair of {reader,writer}
// configurations,
// // //   // because any built-in FilterPolicy should be able to read filters
// from any
...
// // //       // FULL_POSITIVE does not include block-based filter case (j ==
// 0)
// // //       EXPECT_EQ(TestGetAndResetTickerCount(options,
// BLOOM_FILTER_FULL_POSITIVE),
// // //                 j == 0 ? 0 : 2);
// // //       EXPECT_EQ(TestGetAndResetTickerCount(options,
// BLOOM_FILTER_USEFUL), 1);
// // //     }
// // //   }
// // // }
 ...
 public:
  DBFilterConstructionReserveMemoryTestWithParam()
    ...
    if (!reserve_table_builder_memory_) {
      ...
      // TODO: Add support for this test for our filter !!!!!!!!!!!!!!!!!!
    }
  }
...
TEST_P(DBFilterConstructionReserveMemoryTestWithParam, ReserveMemory) {
  // // // Options options = CurrentOptions();
  // // // // We set write_buffer_size big enough so that in the case where
  // there is
  // // // // filter construction cache reservation, flush won't be triggered
  // before we
  ...
  // // //        "construction cache reservation - please make sure no flush
  // triggered "
  ...
  // // // std::string policy = kSpdbPairedBloom;
  ...
  // // //     << "It's by this test's design that
  // predicted_hash_entries_cache_res is "
  ...
  // // //         std::ceil(1.0 *
  // predicted_hash_entries_cache_res_dummy_entry_num / 6 * 1)) *
  // kDummyEntrySize;
  // // // const std::size_t predicted_banding_cache_res =
  // // //     static_cast<std::size_t>(
  // // //         std::ceil(predicted_hash_entries_cache_res_dummy_entry_num
  // * 2.5)) *
  // // //     kDummyEntrySize;
...
class DBFilterConstructionCorruptionTestWithParam
 ...
 public:
  ...
  BlockBasedTableOptions GetBlockBasedTableOptions() {
    ...
    table_options.partition_filters = std::get<1>(GetParam());
    if (table_options.partition_filters) {
      table_options.index_type =
          BlockBasedTableOptions::IndexType::kTwoLevelIndexSearch;
...
INSTANTIATE_TEST_CASE_P(DBFilterConstructionCorruptionTestWithParam,
                        DBFilterConstructionCorruptionTestWithParam,
...
// // //  public:
// // //   explicit LevelAndStyleCustomFilterPolicy(int bpk_fifo, int
// bpk_l0_other,
// // //                                            int bpk_otherwise)
...
// // //  public:
// // //   explicit TestingContextCustomFilterPolicy(int bpk_fifo, int
// bpk_l0_other,
// // //                                             int bpk_otherwise)
// // //       : LevelAndStyleCustomFilterPolicy(bpk_fifo, bpk_l0_other,
// bpk_otherwise) {
// // //   }
...
// // //     test_report_ += ",l=";
// // //     test_report_ +=
// ROCKSDB_NAMESPACE::ToString(context.level_at_creation);
// // //     test_report_ += ",b=";
// // //     test_report_ +=
// ROCKSDB_NAMESPACE::ToString(int{context.is_bottommost});
// // //     test_report_ += ",r=";
// // //     test_report_ +=
// table_file_creation_reason_to_string[context.reason];
// // //     test_report_ += "\n";

// // //     return
// LevelAndStyleCustomFilterPolicy::GetBuilderWithContext(context);
// // //   }
...
// // //               fifo ?
// "cf=abe,s=kCompactionStyleFIFO,n=1,l=0,b=0,r=kFlush\n"
// // //                    :
...
// // //       ASSERT_EQ(Key(i), Get(1, Key(i)));
// // //     }
// // //     // Since we have two tables / two filters, we might have Bloom
// checks on
// // //     // our queries, but no more than one "useful" per query on a found
// key.
// // //     EXPECT_LE(TestGetAndResetTickerCount(options, BLOOM_FILTER_USEFUL),
// maxKey);
...
// // //       // Full compaction
// // //       ASSERT_OK(db_->CompactRange(CompactRangeOptions(), handles_[1],
// nullptr,
// // //                                   nullptr));
// // //       EXPECT_EQ(policy->DumpTestReport(),
// // // "cf=bob,s=kCompactionStyleLevel,n=7,l=1,b=1,r=kCompaction\n");

// // //       // Check that we now have one filter, about 9.2% FP rate (5 bits
// per key)
// // //       for (int i = 0; i < maxKey; i++) {
// // //         ASSERT_EQ("NOT_FOUND", Get(1, Key(i + 33333)));
// // //       }
...
// // //       }
// // //       // Note: kCompactionStyleLevel is default, ignored if num_levels
// == -1
// // //       EXPECT_EQ(policy->DumpTestReport(),
...
TEST_F(SpdbDBBloomFilterTest, PrefixScan) {
  while (ChangeFilterOptions()) {
    ...
    BlockBasedTableOptions table_options;
    table_options.no_block_cache = true;
    table_options.filter_policy.reset(new SpdbPairedBloomFilterPolicy(20));
...
TEST_F(SpdbDBBloomFilterTest, DynamicBloomFilterUpperBound) {
  ...
  table_options.filter_policy = Create(20, bfp_impl);
...
TEST_F(SpdbDBBloomFilterTest, DynamicBloomFilterNewColumnFamily) {
  ...
  table_options.filter_policy = Create(20, bfp_impl);
  options.table_factory.reset(NewBlockBasedTableFactory(table_options));
  CreateAndReopenWithCF({"pikachu0"}, options);
  ReadOptions read_options;
  read_options.prefix_same_as_start = true;
  // create a new CF and set prefix_extractor dynamically
  options.prefix_extractor.reset(NewCappedPrefixTransform(3));
  CreateColumnFamilies({"ramen_dojo_0"}, options);
  ...
  dbfull()->Flush(FlushOptions());
  {
    std::unique_ptr<Iterator> iter(db_->NewIterator(read_options, handles_[2]));
...
// // // TEST_F(SpdbDBBloomFilterTest, SeekForPrevWithPartitionedFilters) {
// // //   Options options = CurrentOptions();
...
// // //   options.compression = kNoCompression;
// // //   BlockBasedTableOptions bbto;
// // //   bbto.filter_policy.reset(new SpdbPairedBloomFilterPolicy(50));
...
// // //   for (size_t i = 0; i < kNumKeys; ++i) {
// // //     // Seek with a key after each one added but with same prefix. One
// will
