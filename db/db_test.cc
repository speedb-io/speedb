...
TEST_F(DBGsTest, GS_RangeTsInImmCoveringValueInMutableButIgnored) {
  ReopenNewDb();
  auto dflt_cfh = dbfull()->DefaultColumnFamily();
...
TEST_F(DBGsTest, GS_RangeTsInImmNotCoveringValueInMutable) {
  ...
  ASSERT_OK(dbfull()->DeleteRange(WriteOptions(), dflt_cfh, "c", "z"));
  ASSERT_OK(dbfull()->TEST_SwitchMemtable());
  ASSERT_OK(dbfull()->Put(WriteOptions(), "c", "b1"));
  ASSERT_OK(dbfull()->Put(WriteOptions(), "a", "a1"));
...
TEST_F(DBGsTest, GS_RangeTsAndDelKeyInImmCoveringInL0) {
  ReopenNewDb();
  auto dflt_cfh = dbfull()->DefaultColumnFamily();
