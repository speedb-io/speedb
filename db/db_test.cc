...
TEST_F(DBTest, SkipDelay) {
  ...
  for (bool sync : {true, false}) {
    for (bool disableWAL : {true, false}) {
      ...
      // the test flaky
      auto token = dbfull()->write_controller()->GetDelayToken(1);
      ...
      token = dbfull()->write_controller()->GetDelayToken(1000000);
...
TEST_F(DBTest, MixedSlowdownOptionsStop) {
  ...
  // TODO(myabandeh): this is time dependent and could potentially make
  // the test flaky
  auto token = dbfull()->write_controller()->GetStopToken();
  std::atomic<int> wait_count(0);
  ROCKSDB_NAMESPACE::SyncPoint::GetInstance()->SetCallBack(
...
TEST_F(DBTest, DynamicCompactionOptions) {
  ...
  while (count < 64) {
    ...
    ASSERT_OK(dbfull()->TEST_FlushMemTable(true, true));
    count++;
    if (dbfull()->write_controller()->IsStopped()) {
      for (auto& sleeping_task : sleeping_task_low) {
