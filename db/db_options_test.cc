...
TEST_F(DBOptionsTest, EnableAutoCompactionAndTriggerStall) {
  ...
  for (int method_type = 0; method_type < 2; method_type++) {
    for (int option_type = 0; option_type < 4; option_type++) {
      ...
      Reopen(options);
      ASSERT_OK(dbfull()->TEST_WaitForCompact());
      ASSERT_FALSE(dbfull()->write_controller()->IsStopped());
      ASSERT_FALSE(dbfull()->write_controller()->NeedsDelay());
      ...
      switch (option_type) {
        case 0:
          ASSERT_TRUE(dbfull()->write_controller()->IsStopped());
          break;
        case 1:
          ASSERT_FALSE(dbfull()->write_controller()->IsStopped());
          ASSERT_TRUE(dbfull()->write_controller()->NeedsDelay());
          break;
        case 2:
          ASSERT_TRUE(dbfull()->write_controller()->IsStopped());
          break;
...
TEST_F(DBOptionsTest, SetBackgroundCompactionThreads) {
  ...
  ASSERT_EQ(1, dbfull()->TEST_BGCompactionsAllowed());
  auto stop_token = dbfull()->write_controller()->GetStopToken();
...
TEST_F(DBOptionsTest, SetDelayedWriteRateOption) {
  ...
  ASSERT_EQ(2 * 1024U * 1024U,
            dbfull()->write_controller()->max_delayed_write_rate());

  ASSERT_OK(dbfull()->SetDBOptions({{"delayed_write_rate", "20000"}}));
  ASSERT_EQ(20000, dbfull()->write_controller()->max_delayed_write_rate());
}
